...
//stoppage table save data view from button show korar jonno
    ...
    {
        $data = DB::select(DB::raw("SELECT stoppages.*,locations.location FROM stoppages left join locations on locations.id = stoppages.location_id  where stoppages.route_id = $request->route_id"));
